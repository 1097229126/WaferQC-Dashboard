...
- **分页查询**：支持大量数据的分页浏览
- **可视化看板**：提供7个图表的全面质量监控视图
  - 3个饼图：浓度等级分布、厚度等级分布、综合等级分布
  - 4个折线图：浓度点位一致性、厚度点位一致性、浓度均匀性、厚度均匀性
...
│   ├── src/
│   │   ├── views/            # 页面视图
│   │   │   ├── WaferListView.vue        # 外延片检测大表
│   │   │   ├── WaferManagementView.vue  # 晶片列表
│   │   │   └── DashboardView.vue        # 可视化看板
│   │   ├── components/       # 可复用组件
│   │   ├── api/              # API 调用
│   │   │   └── index.js      # API 客户端
│   │   ├── router/           # 路由配置
...
**注意**：如果数据库中还没有数据，建议先通过"外延片检测大表"页面导入一些测试数据，或者使用批量创建功能添加测试数据。
...
### 可视化看板
- `GET /api/v1/dashboard/grade-distribution` - 获取等级分布统计数据（用于饼图）
- `GET /api/v1/dashboard/wafer-details` - 获取晶片详细测量数据（用于折线图，支持分页）
...
可视化看板提供7个图表，全面展示半导体外延片的质量监控数据。

### 饼图系列（等级分布）

#### 1. 浓度等级饼图
展示所有晶圆的浓度等级分布情况：
- **A级**：浓度均匀性 ≤ 2% 且 浓度Tolerance% ≤ 2%
- **B级**：浓度均匀性 2%~5% 或 浓度Tolerance% 2%~5%
- **不合格**：浓度均匀性 > 5% 或 浓度Tolerance% > 5%

#### 2. 厚度等级饼图
展示所有晶圆的厚度等级分布情况：
- **A级**：厚度均匀性 ≤ 2% 且 厚度Tolerance% ≤ 2%
- **B级**：厚度均匀性 2%~5% 或 厚度Tolerance% 2%~5%
- **不合格**：厚度均匀性 > 5% 或 厚度Tolerance% > 5%

#### 3. 综合等级饼图
展示所有晶圆的综合等级分布情况：
- **A级**：浓度等级和厚度等级均为 A
- **B级**：浓度等级和厚度等级均不为不合格，且至少有一个为 B
- **不合格**：浓度等级或厚度等级中有一个为不合格

### 折线图系列（点位分析）

#### 4. 浓度点位一致性折线图
- **展示内容**：每50组晶片生成一张图，展示25个点位（P1-P25）的浓度单点差值率波动
- **X轴**：25个浓度点位（P1, P2, ..., P25）
- **Y轴**：差值率 = ((设备1.Pi - 设备2.Pi) - 1) × 100%
- **每条线**：代表一个晶片的25个点位的一致性变化
- **分页**：支持翻页查看不同批次的晶片

#### 5. 厚度点位一致性折线图
- **展示内容**：每50组晶片生成一张图，展示25个点位（T1-T25）的厚度单点差值率波动
- **X轴**：25个厚度点位（T1, T2, ..., T25）
- **Y轴**：差值率 = ((设备1.Ti - 设备2.Ti) - 1) × 100%
- **每条线**：代表一个晶片的25个点位的一致性变化
- **分页**：支持翻页查看不同批次的晶片

#### 6. 浓度均匀性折线图
- **展示内容**：每50组晶片生成一张图，展示25个点位（P1-P25）的浓度值分布
- **X轴**：25个浓度点位（P1, P2, ..., P25）
- **Y轴**：浓度测量值（atoms/cm³），使用科学计数法显示（X.XX×10^Y）
- **两条线**：分别代表设备1（蓝色）和设备2（橙色）的测量值
- **分页**：支持翻页查看不同批次的晶片

#### 7. 厚度均匀性折线图
- **展示内容**：每50组晶片生成一张图，展示25个点位（T1-T25）的厚度值分布
- **X轴**：25个厚度点位（T1, T2, ..., T25）
- **Y轴**：厚度测量值（μm）
- **两条线**：分别代表设备1（蓝色）和设备2（橙色）的测量值
- **分页**：支持翻页查看不同批次的晶片
...
1. **访问看板**：点击左侧菜单"可视化看板"进入
2. **查看饼图**：三个饼图自动加载，展示整体等级分布
3. **查看折线图**：
   - 每个折线图默认显示前50个晶片的数据
   - 使用"上一页"/"下一页"按钮切换不同的晶片批次
   - 鼠标悬停可查看具体数值
4. **颜色说明**：
   - 饼图：绿色=A级，黄色=B级，红色=不合格
   - 折线图：蓝色=设备1，橙色=设备2
