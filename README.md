...
#### 浓度均匀性（基于设备1）
使用变异系数（Coefficient of Variation）计算：
```
浓度均匀性 = |STDEV(P1:P25)| / AVG(P1:P25) × 100%
```
其中：
- `STDEV`: 样本标准差（stddev_samp）
- `AVG`: 平均值
- 仅计算设备1的浓度数据
...
同样使用变异系数计算：
```
厚度均匀性 = |STDEV(T1:T25)| / AVG(T1:T25) × 100%
```
其中：
- `STDEV`: 样本标准差（stddev_samp）
- `AVG`: 平均值
...
- 如果均值为0，则均匀性返回 null
- 如果没有有效数据点，返回 null
- **0值处理**：所有测量值为0的点位不参与统计计算（均值、最大值、最小值、标准差等）
- 前端显示时保留两位小数并添加 % 后缀
...
```
点位一致性 = ((设备1.Pi - 设备2.Pi) - 1) × 100%
浓度点位一致性 = AVG(所有25个点位的一致性)
```

#### 厚度点位一致性
对25个厚度点位（T1-T25），计算每个点位的一致性百分比，然后取均值：
```
点位一致性 = ((设备1.Ti - 设备2.Ti) - 1) × 100%
厚度点位一致性 = AVG(所有25个点位的一致性)
```
...
在所有统计计算中，测量值为0的数据点将被自动排除：
- **平均值计算**：排除值为0的测量点
- **最大值/最小值计算**：排除值为0的测量点
- **标准差计算**：排除值为0的测量点
- **均匀性计算**：基于排除0值后的数据进行计算
- **Tolerance%计算**：基于排除0值后的最大值和最小值进行计算

**原因**：0值通常表示测量失败或数据缺失，不应参与统计计算，否则会导致结果失真。

### 质量等级标准

#### 浓度等级（基于设备1数据）
- **A 级**: 浓度均匀性 ≤ 2% 且 浓度Tolerance% ≤ 2%
- **B 级**: 浓度均匀性 2%~5% 或 浓度Tolerance% 2%~5%
- **不合格**: 浓度均匀性 > 5% 或 浓度Tolerance% > 5%

#### 厚度等级（基于设备1数据）
- **A 级**: 厚度均匀性 ≤ 2% 且 厚度Tolerance% ≤ 2%
- **B 级**: 厚度均匀性 2%~5% 或 厚度Tolerance% 2%~5%
- **不合格**: 厚度均匀性 > 5% 或 厚度Tolerance% > 5%

#### 综合等级
- **A 级**: 浓度等级和厚度等级均为 A
- **B 级**: 浓度等级和厚度等级均不为不合格，且至少有一个为 B
- **不合格**: 浓度等级或厚度等级中有一个为不合格
